...
def analyze_payloads(payloads: list[dict[str, Any]], update_baseline: bool = False) -> dict[str, Any]:
    ...
    # Check if static probes actually ran
    static_probes_ran = len(all_findings) > 0
    if not static_probes_ran:
        risk_band = "UNKNOWN"
        blast_contributors.append("Static authority data unavailable")
    else:
